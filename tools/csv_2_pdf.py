...
class CsvToPdfTool(Tool):
    """
    CSV to PDF Converter with Smart Layout Engine.
    Features: Auto-sizing, Landscape support, Table splitting for wide columns.
    """

    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage, None, None]:
        if not DEPENDENCIES_AVAILABLE:
            yield self.create_text_message("Error: Required libraries (pandas, reportlab) are missing.")
            return
        ...
        if not input_file.extension or input_file.extension.lower() not in ['.csv']:
            yield self.create_text_message("Error: Only .csv files are supported.")
            return
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                # 5. 执行转换核心逻辑
                converter = CsvPdfConverter(input_path, output_path)
                ...
                yield self.create_text_message(f"Successfully converted CSV to PDF: {output_filename}\n{result['message']}")
...
class CsvPdfConverter:
    ...
    def convert(self) -> Dict[str, Any]:
        try:
            ...
            # 添加标题
            story.append(Paragraph(f"CSV File: {title}", title_style))
            ...
            rows, cols = df.shape
            return {"success": True, "message": f"PDF created successfully with {rows} rows and {cols} columns using {used_encoding} encoding"}
