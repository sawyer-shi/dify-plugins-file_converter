...
class WordToPdfTool(Tool):
    ...
    def _process_conversion(self, file_info: Dict[str, Any], temp_dir: str) -> Dict[str, Any]:
        """Process the Word to PDF conversion."""
        input_path = file_info["path"]
        output_files = []
        
        try:
            if not word:
                return {"success": False, "message": "python-office library is not available for Word conversion"}
            ...
            # Convert Word to PDF
            word.docx2pdf(input_path, output_path)
            output_files.append(output_path)
